...
int main() {
	...
	do {
		...
		//Desactivo la escritura en el ColorBuffer y el DepthBuffer
		//para modificar el stencil sin afvectar ni a la imagen ni a la info de la profundidad
		glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
		glDepthMask(GL_FALSE);
		
		//siempre pasa el test de stencil y escribe el valor 1 en el Stencil Buffer
		glStencilFunc(GL_ALWAYS, 1, ~0);
		//cuando se pasa el test, se debe reemplazar el valor en el stencil con 1
		glStencilOp(GL_REPLACE, GL_REPLACE, GL_REPLACE);
		//dibuja el piso encendido
		drawFloor(true);
		
		//solo permite pasar el test de stencil si el valor actual del Stencil Buffer es 1
		glStencilFunc(GL_EQUAL, 1, ~0);
		//mantiene el valor del stencil cuando se pasa o falla el test de profundidad, 
		//pero incrementa el valor de stencil en 1 cuando se pasa el test de stencil
		glStencilOp(GL_KEEP, GL_KEEP, GL_INCR);
		//dibuja el objeto como sombra
		drawObject(shadow);
		
		glColorMask(GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE);
		glDepthMask(GL_TRUE);
		
		//solo dibuja el reflejo donde el Stencil Buffer tiene el valor 1
		glStencilFunc(GL_EQUAL, 1, ~0);
		//mantiene los valores del stencil sin modificarlos
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP);
		drawObject(reflection);
		drawFloor(true);
		
		//permite dibujar solo en las areas donde el stencil tiene el valor 2
